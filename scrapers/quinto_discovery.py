...
def parse_sitemap_index(xml_text: str) -> List[str]:
    return [
        ...
        if entry.get("loc") and re.search(r"/sitemap-v2-listings-part-\d+\.xml$", str(entry["loc"]))
    ]


def _derive_business_type_from_listing_url(listing_url: str) -> str | None:
    if "/alugar/" in listing_url:
        return "rent"
    if "/comprar/" in listing_url:
        return "sale"
    return None


def _derive_listing_id_from_listing_url(listing_url: str) -> str | None:
    match = re.search(r"/imovel/(\d+)/", listing_url)
    if not match:
        return None
...
def collect_discovery_records(verbose: bool = False) -> List[Dict[str, str | None]]:
    ...
    sitemap_index = _fetch_text(session, SITEMAP_INDEX_URL)
    sitemap_urls = parse_sitemap_index(sitemap_index)
    all_records: List[Dict[str, str | None]] = []
    for sitemap_url in sitemap_urls:
        if verbose:
            print(f"[INFO] quinto_discovery_sitemap url={sitemap_url}")
        sitemap_xml = _fetch_text(session, sitemap_url)
...
def collect_discovery_to_file(
    *,
    ...
    if not records:
        print("[WARN] QuintoAndar discovery sem dados coletados")
        return None
